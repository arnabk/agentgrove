...
/// user has set `base_url` + `default_model` but not yet pasted a
/// key, OR when the row's ciphertext failed to decrypt (logged + the
/// caller is expected to treat the provider as unconfigured).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ProviderSecret {
    /// Stable id (e.g. `"9router"`).
    pub provider_id: String,
    /// Base URL the FE / agents talk to (e.g. `http://localhost:20128/v1`).
    pub base_url: String,
    /// Default model the FE seeds new chats with for this provider.
    /// `None` ⇒ no preference; FE picks the first /v1/models entry.
...
impl ProviderSecretRepo {
    ...
    pub async fn get(
        ...
        let row: Option<(String, String, Option<String>, Option<String>, Option<String>)> =
            sqlx::query_as(
                "SELECT provider_id, base_url, default_model, ciphertext_b64, nonce_b64 \
                 FROM provider_secrets WHERE provider_id = ?1",
            )
            .bind(provider_id)
            .fetch_optional(&self.pool)
            .await?;
        let Some((id, base_url, default_model, ct, nonce)) = row else {
            return Ok(None);
        };
    ...
    pub async fn put(
        ...
        // from the UPDATE. SQLite's ON CONFLICT DO UPDATE with
        // `excluded.*` makes this clean.
        match (ct, nonce) {
            (Some(c), Some(n)) => {
                sqlx::query(
                    "INSERT INTO provider_secrets \
                     (provider_id, base_url, default_model, ciphertext_b64, nonce_b64, updated_at) \
                     VALUES (?1, ?2, ?3, ?4, ?5, ?6) \
    ...
    pub async fn list_summaries(
        &self,
    ) -> Result<Vec<ProviderSecretSummary>, ProviderSecretError> {
        let rows: Vec<(String, String, Option<String>, Option<String>)> = sqlx::query_as(
            "SELECT provider_id, base_url, default_model, ciphertext_b64 \
        ...
        Ok(rows
            .into_iter()
            .map(|(provider_id, base_url, default_model, ct)| ProviderSecretSummary {
                provider_id,
                base_url,
                default_model,
...
mod tests {
    ...
    async fn put_then_get_returns_plaintext_key() {
        ...
        repo.put("9router", "http://localhost:20128/v1", Some("free-combo"), Some("sk-xxx"))
            .await
            .unwrap();
        let got = repo.get("9router").await.unwrap().unwrap();
        assert_eq!(got.base_url, "http://localhost:20128/v1");
        assert_eq!(got.default_model.as_deref(), Some("free-combo"));
    ...
    async fn put_with_no_api_key_preserves_existing_key() {
        let (_tmp, repo) = fixture().await;
        repo.put("9router", "http://a", None, Some("sk-1")).await.unwrap();
        // Update base_url without changing the key.
        repo.put("9router", "http://b", None, None).await.unwrap();
        let got = repo.get("9router").await.unwrap().unwrap();
    ...
    async fn put_with_empty_string_key_clears_existing_key() {
        let (_tmp, repo) = fixture().await;
        repo.put("9router", "http://a", None, Some("sk-1")).await.unwrap();
        repo.put("9router", "http://a", None, Some("")).await.unwrap();
        let got = repo.get("9router").await.unwrap().unwrap();
    ...
    async fn list_summaries_omits_keys() {
        let (_tmp, repo) = fixture().await;
        repo.put("9router", "http://a", Some("m1"), Some("sk-1")).await.unwrap();
        repo.put("openai", "http://b", None, None).await.unwrap();
        let sums = repo.list_summaries().await.unwrap();
        assert_eq!(sums.len(), 2);
        let nine = sums.iter().find(|s| s.provider_id == "9router").unwrap();
        assert!(nine.has_api_key);
        let oai = sums.iter().find(|s| s.provider_id == "openai").unwrap();
